...
> **重要更新**：從 2026-05-25 起，表情系統已整合到 `actions.json` 配置驅動架構。本文件的**程式碼範例僅供參考表情參數設計**，實際使用請編輯 `actions.json`。

## 快速開始

表情與動作現在都在 `actions.json` 中定義。範例：
...
```json
{
  "my_action": [
    ...
    { "type": "motion", "yaw": 0, "pitch": 450, "speed": 400, "delay": 0 }
  ]
}
```

更多細節請參考本文件的**參數規格**與**表情設計範例**章節。
...
## 在 actions.json 中使用表情

### 基本語法

表情步驟使用 `type: "avatar"` 並提供 `leftEye`, `rightEye`, `mouth` 參數：
...
可以在同一個動作序列中混合使用 `motion` 和 `avatar`：

```json
{
  "happy_nod": [
    { "type": "avatar", "leftEye": {...}, "rightEye": {...}, "mouth": {...} },
    { "type": "motion", "yaw": 0, "pitch": 200, "speed": 400, "delay": 0 },
    { "type": "motion", "yaw": 0, "pitch": 450, "speed": 400, "delay": 400 }
  ]
}
```
...
> **⚠️ 此章節已過時**：目前架構使用 `ActionController` 序列執行機制，不再支援 `setInterval` 持續動畫。如需類似效果，請在 `actions.json` 中定義多步驟序列。
...
透過 Bridge Server 的 `/action` 端點觸發預定義動作：
...
這會執行 `actions.json` 中定義的 `thinking` 動作序列（包含表情與動作）。

### 直接發送表情（進階）

> **不推薦**：直接呼叫 Provider API 會繞過 ActionController 的取消機制，可能導致動作重疊。

如需直接控制表情，可透過 `/avatar` 端點（需先在 `server.js` 實作此端點）：
